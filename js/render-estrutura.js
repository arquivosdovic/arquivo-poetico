...
import { reordenarPosicao, seqOuNull, fecharEspaco,
         getIrmaosPorEscopo, escapeHtml, mostrarAviso } from './utils.js';
...
export function exportarSelecaoEstrutura() {
    ...
    if (total === 0) { mostrarAviso('Nenhum item selecionado.'); return; }
...
function renderNoEstrutura({ tipo, dados }, nivel) {
    ...
    if (tipo === 'elemento') {
        ...
        titulo = dados.titulo ? `${escapeHtml(rotuloTipo)}: ${escapeHtml(dados.titulo)}` : escapeHtml(rotuloTipo);
    }
...
function colecoesMoviveis() {
    return {
        secao: db.secoes, poema: db.poemas, prosa: db.prosas, elemento: db.elementos
    };
}
